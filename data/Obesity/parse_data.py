...
for line in open("abundance_obesity.txt"):
    split = line.split("\t")
    otu = split[0].split("g__")[1]
    otu = otu.split("_noname")[0].replace("_", " ").replace("unclassified","").replace("Clostridiales Family XI Incertae Sedis", "Clostridiales").replace("Clostridiales Family XIII Incertae Sedis", "Clostridiales").strip()
