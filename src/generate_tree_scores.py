...
for i in scores_diff.index.values:
	if abs(quantile_rank_df[labels[0]].loc[i].as_matrix() - quantile_rank_df[labels[1]].loc[i].as_matrix()) <= 20:
		sign_list.append(0)
	elif quantile_rank_df[labels[0]].loc[i].as_matrix() < quantile_rank_df[labels[1]].loc[i].as_matrix() and quantile_rank_df[labels[0]].loc[i].as_matrix():
		sign_list.append(-1)
	else:
		sign_list.append(1)
