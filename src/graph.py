...
class Graph:
	...
	def raw_vector(self, raw_vector):
		out_vec = []
		for otu in raw_vector:
			try:
				node = self.get_node_by_name(otu)
				out_vec.append(node.get_abundance())
			except:
				pass
		return out_vec
	...
	def populate_graph(self, lab, x):
		...
		for i in range(0, len(list(lab.index))):
			abundance = x[i]
			label = lab.iloc[i][level]
			if "noname" in label:
				label = label.split("_noname")[0]
			if label == "NA":
				level = 'genus'
			if label == "NA":
				level = 'family'
			...
			if node == None:
				label = label.split("unclassified_")[-1]
				node = self.get_node_by_name(label)
			try:
				node.set_abundance(float(node.get_abundance()) + float(abundance))
