...
def PopPhyCNN1D(num_features, num_class):
	...
	with tf.variable_scope("Output"):
		...
		fc1 = tf.layers.dense(conv_flat, 32, activation=tf.nn.relu, kernel_regularizer=regularizer, bias_regularizer=regularizer)
...
def train():
	...
	for set in range(0, num_set):
		for cv in range(0,num_cv):
			...
			train = x.values.reshape(x.shape[0], 1, num_features, 1)
			test = tx.values.reshape(tx.shape[0], 1, num_features, 1)
			...
			valid_x = valid_x.reshape(-1, num_features)
			test = test.reshape(-1, num_features)
			...
			train_x = train_x.reshape(-1, 1, num_features, 1)
			valid_x = valid_x.reshape(-1, 1, num_features, 1)
			test_x = test_x.reshape(-1, 1, num_features, 1)
			...
			model = PopPhyCNN1D(num_features, num_class)
			...
			with tf.Session() as sess:
					...
					while True:
						try:
							...
							if stat == "AUC" and roc > best_val:
								...
								trained_epochs = i
								best_val = roc
								save_path = saver.save(sess, dir+"/1D-CNN.ckpt")	
				...
				saver.restore(sess, dir+"/1D-CNN.ckpt")
