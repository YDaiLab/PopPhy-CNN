	...
	for cv in range(0,args.splits):
		...
		for run in range(0,1):
			...
			if classifier=="SVM":
				...
				clf = GridSearchCV(SVC(C=1, probability=True), param_grid=cv_grid, cv=StratifiedKFold(y[0], n_folds=5, shuffle=True), n_jobs=-1, scoring="roc_auc")
				clf.fit(x, y[0])
				prob = [row[1] for row in clf.predict_proba(tx)]
				pred = [row for row in clf.predict(tx)]
			...
			if run_auc > best_auc and stat == "AUC":
				best_auc = run_auc
				...
				best_clf = clf	
                        ...
                        if run_mcc > best_mcc and stat == "MCC":
                                best_auc = run_auc
                                best_mcc = matthews_corrcoef(ty, pred)
                                best_acc = clf.score(tx,ty)
                                best_precision = precision_score(ty, pred, average='weighted')
