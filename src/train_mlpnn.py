...
def PopPhyCNN(num_features, num_class):
	...
	with tf.variable_scope("Output"):
	    fc = tf.layers.dense(X, 128, activation=tf.nn.relu, kernel_regularizer=regularizer, bias_regularizer=regularizer)
	    fc_drop = tf.layers.dropout(fc, rate=0.2)
	    fc1 = tf.layers.dense(fc_drop, 128, activation=tf.nn.relu, kernel_regularizer=regularizer, bias_regularizer=regularizer)
...
def train():
    ...
    for set in range(0, num_set):
	    ...
	    print("There are %d features" % (num_features))
            train = x.as_matrix().reshape(-1, num_features)
            test = tx.as_matrix().reshape(-1, num_features)
